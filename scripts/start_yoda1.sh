...
export EXECUTOR_URL=http://127.0.0.1:5000/execute
...
yoda config executor "rest:$EXECUTOR_URL?timeout=5000s" --home ~/.yoda1

# setup broadcast-timeout to yoda config
yoda config broadcast-timeout "100m" --home ~/.yoda1
...
echo "y" | bandd tx oracle activate --from validator1 --keyring-backend test --chain-id bandchain --home ~/.band1 --keyring-dir ~/.band1
...
echo "y" | bandd tx bank send validator1 $(yoda keys list -a --home ~/.yoda1) 1000000uband --keyring-backend test --chain-id bandchain  --keyring-dir ~/.band1 --home ~/.band1

# wait for sending band tokens transaction success
sleep 2

# add reporter to bandchain
echo "y" | bandd tx oracle add-reporters $(yoda keys list -a --home ~/.yoda1) --from validator1 --keyring-backend test --chain-id bandchain --keyring-dir ~/.band1 --home ~/.band1
